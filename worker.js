...
async function openTunnel(data, server, cfg) {
  ...
  // VLESS standard: version(1) + UUID(16) + command(1) + port(2) + addrType(1) + addr(variable)
  if (buf[0] !== 0) { server.close(); return null; }
  
  // Check UUID (bytes 1-16)
  const uuidHex = hex(buf.slice(1, 17));
  const expectedHex = cfg.uuid.replace(/-/g, "").toLowerCase();
  if (uuidHex !== expectedHex) { server.close(); return null; }
  
  const command = buf[17]; // 1=TCP, 2=UDP, 3=Mux
  let off = 18; // after version(1) + uuid(16) + command(1)
  ...
  if (at === 1) { // IPv4
    len = 4;
    addr = Array.from(buf.slice(off + 3, off + 7)).join(".");
  } else if (at === 2) { // Domain
    len = buf[off + 3];
    addr = new TextDecoder().decode(buf.slice(off + 4, off + 4 + len));
  } else if (at === 3) { // IPv6
    len = 16;
    const dv = new DataView(buf.buffer, off + 3, 16);
  ...
  const start = off + 3 + len;
  
  // Send VLESS response: version(0) + no add-ons(0)
  ...
  if (start < buf.length) {
    await writer.write(buf.slice(start));
  }
...
function buildSub(cfg, host) {
  ...
  const params = ":443?encryption=none&security=tls&sni=" + host + "&type=ws&host=" + host + "&path=" + cfg.wsPath + "&fp=chrome";
